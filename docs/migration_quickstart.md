...
3. **Deploy new provider images:**
    ...
    - Specify the new images:  
      `ghcr.io/oracle/provider-family-oci:v0.0.1-alpha.1-amd64`  
      `ghcr.io/oracle/provider-oci-objectstorage:v0.0.1-alpha.1-amd64`
...
3. **Delete ProviderConfig (force remove finalizers if needed):**
    ...
    kubectl get providerconfig
     No resources found
    ```
    Deletion makes the associated resource sync to false:
   
   ```sh
   $ k get managed 
    NAME                                                         SYNCED   READY   EXTERNAL-NAME                            AGE
    bucket.objectstorage.oci.upbound.io/bucket-via-crossplane4   False    True    n/iddevjmhjw0n/b/bucket-via-crossplane   17m
    ```
...
7. **Delete the old family provider**  
   (_Caution: Check the provider name and do not delete the newly created family provider_)
...
8. **Recreate ProviderConfig:**
    ...
    Creation should turn sync to true for the associated resource:
